...
ACTIVITY_TYPES    = ["study","buried","reside","visit","born","died","marry","hajj","other"]
LAQAB_TYPES       = ["laqab","shuhrah","kunyah"]
LAQAB_LABELS      = {"laqab":"laqab（号）","shuhrah":"shuhrah（通称）","kunyah":"kunyah（クンヤ）","honorific":"honorific（敬称）"}
...
FAMILY_RELATIONS = [
    ...
    ("brothers_son",            "Brother's son（兄弟の息子）"),
    ("brothers_daughter",       "Brother's daughter（兄弟の娘）"),
    ("sisters_son",             "Sister's son（姉妹の息子）"),
    ("sisters_daughter",        "Sister's daughter（姉妹の娘）"),
    ("siblings_child",          "Sibling's child, unspecified（兄弟姉妹の子・系統不明）"),
