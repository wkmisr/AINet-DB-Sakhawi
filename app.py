...
def genai_generate_text(prompt, model_name=None):
    """新SDK(google-genai)でテキスト生成し、(text, model_name, error, resolved_version) を返す。
    error は None なら成功。呼び出し側で診断表示・パースを行う。
    resolved_version は API 応答が返す実際のモデルバージョン文字列(取得できない
    SDK/APIバージョンでは None)。model_name は要求時に指定した名前(エイリアス
    の場合あり)、resolved_version はそれが実際に解決した具体バージョン。
    校閲精度をモデル別に遡及検証できるよう、生成物(XMLヘッダーコメント)に
    両方を記録する(2026-09-26、Waka 指示)。"""
    client = get_genai_client()
    if client is None:
        return None, None, "API キーが未設定です(GEMINI_API_KEY)。", None
    name = model_name or get_working_model_name()
    try:
        resp = client.models.generate_content(model=name, contents=prompt)
    except genai_errors.APIError as e:
        return None, name, f"API エラー: {type(e).__name__}: {e}", None
    except Exception as e:
        return None, name, f"{type(e).__name__}: {e}", None
    resolved_version = getattr(resp, "model_version", None) or None
    ...
    return text, name, None, resolved_version
...
DEFAULT_DATA_V19 = {
    ...
    # === 生成元メタデータ(校閲精度の遡及検証用。2026-09-26追加) ===
    # gemini_model_requested: generate_content 呼び出し時に指定したモデル名文字列
    # gemini_model_resolved:  API 応答が返す実際に応答したモデルのバージョン文字列
    #                         (SDK/APIが返さない場合は空のまま)
    "gemini_model_requested": "",
...
"""
    ...
    with st.spinner(f"{len(items)} 項目の翻字を生成中..."):
        try:
            _raw_text, _model_name, _err, _resolved_ver = genai_generate_text(prompt)
            st.caption(f"🔧 使用モデル: {_model_name or '(未取得)'}" + (f" (resolved: {_resolved_ver})" if _resolved_ver else ""))
            if _err:
            ...
            with st.spinner("解析中..."):
                try:
                    ...
                    if prompt is None:
                        ...
                        st.stop()
                    _resp_text, _model_name, _err, _resolved_ver = genai_generate_text(prompt)
                    if _err:
                        st.error(f"解析エラー: {_err}(モデル: {_model_name or '未取得'})")
                        st.stop()
                    # 校閲精度の遡及検証用に、実際に使用した Gemini モデルを
                    # d に保持しておく(build_xml がヘッダーコメントに埋め込む)
                    d["gemini_model_requested"] = _model_name or ""
                    d["gemini_model_resolved"] = _resolved_ver or ""
...
def build_xml(d):
    ...
    # 校閲精度の遡及検証用: 実際に生成に使用した Gemini モデルを記録する
    # (2026-09-26追加。requested=呼び出し時指定名、resolved=API応答の実バージョン)
    _gem_req = (d.get("gemini_model_requested") or "").strip()
    _gem_res = (d.get("gemini_model_resolved") or "").strip()
    _gem_bits = []
    if _gem_req:
        _gem_bits.append(f"gemini-requested={_gem_req}")
    if _gem_res and _gem_res != _gem_req:
        _gem_bits.append(f"gemini-resolved={_gem_res}")
    _gem_suffix = (" " + " ".join(_gem_bits)) if _gem_bits else ""
    x.append(
        f'    <!-- generated by AINet-DB Researcher Pro '
        f'{APP_VERSION} ({APP_VERSION_DATE}) at {_generated_at}{_gem_suffix} -->'
    )
